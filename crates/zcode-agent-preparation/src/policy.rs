...
macro_rules! define_agent_bash_command_families {
    ($($program:literal),+ $(,)?) => {
        ...
        fn agent_bash_program_allowed(program: &str) -> bool {
...
fn valid_agent_git(args: &[String]) -> bool {
    let mut index = 0;
...
fn agent_bash_path_operands(program: &str, args: &[String]) -> Option<Vec<String>> {
    match program {
        "pwd" => args
            .iter()
...
fn tokenize_agent_bash(command: &str) -> Option<Vec<String>> {
    if command.is_empty()
        || command.len() > 16 * 1024
...
fn valid_agent_sed(args: &[String]) -> bool {
