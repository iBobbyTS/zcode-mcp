...
4. restricted to the review root;
...
An unsupported command is not assumed safe. The default decision is `deny`; deployments that prefer an interactive fallback may set `ZCODE_READONLY_BASH_UNKNOWN_DECISION=ask`.
...
The root is `ZCODE_READONLY_BASH_ROOT` when set, otherwise the hook `cwd`.
...
Allows common search, glob, type, context, file-list, and output-format options. `--pre`, `--pre-glob`, and `--follow` are denied. `rg` may default to the current review root when no path is supplied.
...
Allows hashing explicit confined files. Check-file mode is denied because checksum files can refer to paths outside the review root.
