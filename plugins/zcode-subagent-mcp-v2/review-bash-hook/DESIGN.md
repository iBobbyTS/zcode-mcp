...
Safely evaluating `&&`, pipelines, redirections, subshells, variables, globbing, and wrapper shells requires a complete shell parser plus composition rules. That adds a large security surface for little review value. ZCode can issue multiple tool calls for multiple inspections.
