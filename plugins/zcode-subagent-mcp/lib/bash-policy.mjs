...
export const POLICY_VERSION = 'zcode-agent-bash/v1.0.0';
...
const POLICY_DESCRIPTOR = Object.freeze({
  ...
  unknownDefault: 'deny',
  pathScope: 'hook cwd or ZCODE_AGENT_BASH_ROOT',
...
export function validatePathArgument(raw, context, options = {}) {
  ...
  if (!pathInside(context.root, candidate)) {
    return hardDeny('path_escape', 'path escapes the configured agent root');
  }
  ...
  if (!realAncestor || !pathInside(context.root, realAncestor)) {
    return hardDeny('symlink_escape', 'path resolves through a symlink outside the configured agent root');
  }

  const realCandidate = realpathOrNull(candidate);
  if (realCandidate && !pathInside(context.root, realCandidate)) {
    return hardDeny('symlink_escape', 'path resolves outside the configured agent root');
  }
...
function parseGrepLike(program, args, context, mode) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (!optionsEnded && deniedLongPrefixes.some((prefix) => token === prefix || token.startsWith(`${prefix}=`))) {
      return hardDeny(`${mode}_execution_option_denied`, `${token} can invoke external processing or follow paths outside the agent root`);
    }
    if (!optionsEnded && token.startsWith('--')) {
      const { name, inlineValue } = parseLongOption(token);
...
function validateFind(args, context) {
  ...
  while (i < args.length) {
    ...
    if (['-L', '-H'].includes(token)) return hardDeny('find_symlink_follow_denied', `${token} may follow symlinks outside the agent root`);
...
function validateChecksum(program, args, context) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (!optionsEnded && program === 'shasum' && ['-c', '--check'].includes(token)) {
      return hardDeny('checksum_check_file_denied', 'checksum check mode can reference paths outside the agent root');
    }
