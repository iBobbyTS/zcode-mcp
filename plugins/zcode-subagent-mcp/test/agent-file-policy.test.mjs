...
test('installer adds recognized matcher while preserving Bash and Post hooks', () => {
  ...
  const installer = path.resolve(new URL('../scripts/install-agent-hooks.mjs', import.meta.url).pathname);
  const proc = spawnSync(process.execPath, [installer, '--config', config, '--provenance', provenance], { encoding: 'utf8' });
  ...
  assert.equal(installed.hooks.events.PostToolUse[0].hooks[0].args[0].endsWith('/hooks/audit-bash-result.mjs'), true);
  assert.equal(JSON.parse(fs.readFileSync(provenance, 'utf8')).effective_config_sha256.length, 64);
});
